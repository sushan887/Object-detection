$(document).ready(function () {
  ...
  function addDetections(idVideo) {
    ...
    $.getJSON(
      `static/media_saved/json/detected_${idVideo}.json`,
      function (data) {
  ...
  function drawObjectDetected(detection) {
    ...
    let boxHeight =
      ((detection[4] - detection[2]) * canvasHeight) / videoHeight;

    // Draw bounding box
    ctxVideo.beginPath();
    ctxVideo.lineWidth = "2";
    ctxVideo.strokeStyle = "yellow";
  ...
  function showAmountLabels(detections) {
    ...
    countedElements.forEach((element) => {
      detectedObjectsRankingContainer.append(
